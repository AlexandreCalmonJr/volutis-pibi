...
export default function Louvor() {
  ...
  const [holyricsConfig, setHolyricsConfig] = useState<HolyricsConfig | null>(null);
  ...
  const [novaMusica, setNovaMusica] = useState({ title: "", artist: "", originalKey: "", bpm: "", structure: "", youtubeUrl: "", spotifyUrl: "", cifraClubUrl: "" });

  useEffect(() => {
    carregarMusicas();
    carregarEventos();
    if (canManageHolyrics) carregarHolyrics();
  }, []);

  useEffect(() => {
    if (setlistAberta) carregarSetlist(setlistAberta);
  }, [setlistAberta]);
  ...
  async function testarHolyrics() {
    ...
    try {
      const result = await api<{ connected: boolean; version?: string | null; permissions?: string[] | null }>("/holyrics/test", { method: "POST", body: {} });
      setHolyricsStatus({ configured: true, connected: result.connected, version: result.version ?? null, permissions: result.permissions ?? null });
      setHolyricsFeedback({ type: "ok", text: result.connected ? `Conexão OK${result.version ? ` · versão ${result.version}` : ""}` : "Holyrics não respondeu." });
    } catch (err: any) {
  ...
  async function enviarSetlistHolyrics() {
    ...
    setHolyricsFeedback(null);
    try {
      const result = await api<{ sent: number; skipped: string[] }>(`/events/${setlistAberta}/holyrics/send-setlist`, { method: "POST", body: { clear: true } });
      setHolyricsFeedback({ type: "ok", text: `Setlist enviada ao Holyrics (${result.sent} música(s)).${result.skipped.length ? ` Sem vínculo: ${result.skipped.join(", ")}` : ""}` });
    } catch (err: any) {
  ...
  async function adicionarMusica() {
    ...
    try {
      await api("/songs", { method: "POST", body: { title: novaMusica.title.trim(), artist: novaMusica.artist.trim() || undefined, originalKey: novaMusica.originalKey.trim() || undefined, bpm: novaMusica.bpm ? Number(novaMusica.bpm) : undefined, structure: novaMusica.structure.trim() || undefined, youtubeUrl: novaMusica.youtubeUrl.trim() || undefined, spotifyUrl: novaMusica.spotifyUrl.trim() || undefined, cifraClubUrl: novaMusica.cifraClubUrl.trim() || undefined } });
      await carregarMusicas();
      setNovaMusica({ title: "", artist: "", originalKey: "", bpm: "", structure: "", youtubeUrl: "", spotifyUrl: "", cifraClubUrl: "" });
  ...
  const musicasFiltradas = useMemo(() => musicas.filter((m) => m.title.toLowerCase().includes(buscaMusica.toLowerCase()) || (m.artist || "").toLowerCase().includes(buscaMusica.toLowerCase())), [musicas, buscaMusica]);
            ...
            {setlistAberta ? (() => {
              ...
              return <>
                <div className="px-6 py-4 border-b border-[#f0eefe] flex items-center justify-between gap-3" style={{ background: "linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%)" }}>
                  ...
                  {canManageHolyrics && <button onClick={enviarSetlistHolyrics} className="px-4 py-2 rounded-xl bg-[#1e1b4b] text-white text-sm font-semibold">Enviar ao Holyrics</button>}
                </div>
        ...
        <div className="space-y-4">
          ...
          {carregandoMusicas ? <div className="bg-white rounded-2xl border border-[#e5e0f8] p-8 text-center"><p className="text-sm text-[#7c6ea8]">Carregando repertório...</p></div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{musicasFiltradas.map((m) => {
            ...
            return <div key={m.id} className="bg-white rounded-2xl border border-[#e5e0f8] p-5">
              ...
              <div className="mt-4 flex gap-2"><button onClick={() => removerMusica(m.id)} className="px-3 py-2 rounded-xl border border-red-200 text-red-600 text-xs font-semibold">Excluir</button></div>
            </div>;
          ...
          {showAddModal && <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50"><div className="bg-white rounded-2xl p-6 w-full max-w-md space-y-4"><h3 className="font-semibold text-[#1e1b4b]">Nova Música</h3><input value={novaMusica.title} onChange={(e) => setNovaMusica({ ...novaMusica, title: e.target.value })} placeholder="Título *" className="w-full px-4 py-2.5 text-sm border border-[#e5e0f8] rounded-xl" /><input value={novaMusica.artist} onChange={(e) => setNovaMusica({ ...novaMusica, artist: e.target.value })} placeholder="Artista" className="w-full px-4 py-2.5 text-sm border border-[#e5e0f8] rounded-xl" /><div className="grid grid-cols-2 gap-3"><input value={novaMusica.originalKey} onChange={(e) => setNovaMusica({ ...novaMusica, originalKey: e.target.value })} placeholder="Tom" className="w-full px-4 py-2.5 text-sm border border-[#e5e0f8] rounded-xl" /><input value={novaMusica.bpm} onChange={(e) => setNovaMusica({ ...novaMusica, bpm: e.target.value })} placeholder="BPM" type="number" className="w-full px-4 py-2.5 text-sm border border-[#e5e0f8] rounded-xl" /></div><input value={novaMusica.youtubeUrl} onChange={(e) => setNovaMusica({ ...novaMusica, youtubeUrl: e.target.value })} placeholder="YouTube URL" className="w-full px-4 py-2.5 text-sm border border-[#e5e0f8] rounded-xl" /><input value={novaMusica.spotifyUrl} onChange={(e) => setNovaMusica({ ...novaMusica, spotifyUrl: e.target.value })} placeholder="Spotify URL" className="w-full px-4 py-2.5 text-sm border border-[#e5e0f8] rounded-xl" /><input value={novaMusica.cifraClubUrl} onChange={(e) => setNovaMusica({ ...novaMusica, cifraClubUrl: e.target.value })} placeholder="CifraClub URL" className="w-full px-4 py-2.5 text-sm border border-[#e5e0f8] rounded-xl" /><textarea value={novaMusica.structure} onChange={(e) => setNovaMusica({ ...novaMusica, structure: e.target.value })} placeholder="Estrutura" rows={3} className="w-full px-4 py-2.5 text-sm border border-[#e5e0f8] rounded-xl resize-none" /><div className="flex gap-3 justify-end"><button onClick={() => setShowAddModal(false)} className="px-4 py-2 text-sm font-medium text-[#7c6ea8]">Cancelar</button><button onClick={adicionarMusica} disabled={!novaMusica.title.trim() || adicionandoMusica} className="px-4 py-2 rounded-xl text-sm font-semibold text-white disabled:opacity-50" style={{ backgroundColor: "#7c3aed" }}>{adicionandoMusica ? "Salvando..." : "Adicionar"}</button></div></div></div>}
        </div>
      )}

      {aba === "novo" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl border border-[#e5e0f8] p-6 space-y-4"><h2 className="font-semibold text-[#1e1b4b]">Criar Nova Setlist</h2><div><label className="block text-xs font-semibold text-[#7c6ea8] uppercase tracking-wider mb-1.5">Evento</label><select value={setlistAberta || ""} onChange={(e) => setSetlistAberta(e.target.value || null)} className="w-full px-4 py-2.5 text-sm border border-[#e5e0f8] rounded-xl"><option value="">Selecione um evento</option>{eventos.map((e) => <option key={e.id} value={e.id}>{e.title} — {new Date(e.date).toLocaleDateString("pt-BR")}</option>)}</select></div><div><label className="block text-xs font-semibold text-[#7c6ea8] uppercase tracking-wider mb-2">Músicas Selecionadas ({musicasSelecionadas.length})</label>{musicasSelecionadas.length === 0 ? <p className="text-sm text-[#7c6ea8]">Selecione músicas ao lado →</p> : <div className="space-y-2">{musicasSelecionadas.map((id, i) => { const m = musicas.find((x) => x.id === id); if (!m) return null; return <div key={id} className="flex items-center gap-3 bg-[#f5f3ff] rounded-xl px-3 py-2"><span className="text-xs font-bold text-[#7c3aed]">{i + 1}</span><span className="text-sm font-medium text-[#1e1b4b] flex-1">{m.title}</span><button onClick={() => toggleMusica(id)} className="text-[#7c6ea8]">×</button></div>; })}</div>}</div><button onClick={salvarSetlist} disabled={!setlistAberta || musicasSelecionadas.length === 0 || salvandoSetlist} className="w-full py-3 rounded-xl text-sm font-semibold text-white disabled:opacity-50" style={{ backgroundColor: "#7c3aed" }}>{salvandoSetlist ? "Salvando..." : "Salvar Setlist"}</button></div>
